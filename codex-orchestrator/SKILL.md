...
Use the [review brief and re-review criteria](references/briefs-and-repairs.md#verification-and-review-brief) for material changes outside reviewed scope, changed shared invariants, or unresolved accepted findings. Bounded fixes close through Astra's diff review and targeted checks when these establish the contract. Do not commission a confirmatory review. Opus is `medium`, broad/sensitive review included; `xhigh` is the retry after a `medium` review fell short, or when the lead judges a review needs more depth; `max` remains user-gated as defined in [Claude routing](references/claude-reviews.md#review-routing).
