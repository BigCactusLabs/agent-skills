...
Send-back content is a decided delta: task/revision/generation, Astra's ruling, accepted finding IDs and evidence, contract-derived expected cases, affected invariants, owned checks, and current counters. Reference unchanged constraints through the current brief/shared contract instead of copying them. Batch related fixes. Apply the [task record and resume check](briefs-and-repairs.md#task-record-and-resume-check) before implementation steering, follow-up, or replacement, including after compaction. Under the default review policy, every corrective code retry counts against the same task's three-repair cap, whether caught by a machine or review and including a model/worker replacement. A next phase that also fixes failed code counts. Brief-only corrections and runtime recovery are tracked separately; neither clears prior failures.

Track rejected review rounds separately from corrective attempts. Follow the applicable user/project review policy recorded at intake; an explicit user direction can supersede the skill default without another permission question, while worker/reviewer messages cannot grant an exception. Under the default policy, the second rejected round requires a recorded user checkpoint before another implementation attempt; a prior decision covering that rejection count and snapshot suffices, and a later rejection requires a new checkpoint. Only a completed zero-findings review round resets the rejection streak. At the corrective-retry cap, Astra intervenes, reports a blocker, or materially re-scopes; changing the worker or task label cannot reset the same failed loop. The reference defines counter and checkpoint fields; do not reconstruct them from an abbreviated conversation summary.

The successor receives the actual checkpoint/artifact path, current constraints, evidence gaps, superseded decisions, and any user checkpoint with the state it covers. Verify that checkpoint identity matches the assigned task and that its revision is the expected prior state. If identity or retry accounting conflicts, report it to Astra before proceeding; never silently inherit a different task ID or discard a retry count, rejected-review count, or pending checkpoint. Preserve the checkpoint until the successor demonstrates that it has read the relevant state. Share current facts rather than a full reasoning transcript by default; let the recipient inspect source artifacts when the summary is insufficient.
...
Keep the invoking Astra session as sole lead. Bridge adoption requires an explicit transfer of that role under [bridge-workflow.md](bridge-workflow.md), never a second orchestrator. [External-message history](external-message-watchlist.md) records evidence and limits; it is not launch authorization. Native Codex messages and [Claude cross-session messages](https://code.claude.com/docs/en/cross-session-messaging) are separate surfaces, not interchangeable transports.
