...
Use the shell tool's managed process/session handle to run in the background while Astra does independent work. Do not combine `-p` with `--bg`, append `&`, or confuse the shell session ID with Claude's conversation `session_id`. Track both IDs separately. Preserve the complete yielded tool result, including its managed process handle, instead of printing only `.output`; read the saved log for a tail rather than launching another process. Poll with waits no longer than 60 seconds and preserve stdout/stderr artifacts.
...
Use a fresh conversation for a new independent review. Re-review only when a repair adds a mechanism, code path, error class, or shared invariant; focus on that change and affected invariants. Enumerated fixes get Astra's own review and targeted checks; do not request a confirmatory yes. Astra records one adjudicated rejection per review round and applies the [recorded review policy and resume check](briefs-and-repairs.md#task-record-and-resume-check) before sending any repair. Claude never fixes its own findings in this workflow.
