...
Implementation dispatch uses the [worker brief](briefs-and-repairs.md#worker-brief): `gpt-6-sol` with `high` for machine-checked legs, `gpt-6-astra` with `low` for review-checked legs. Terra and the `gpt-5.6-*` tiers are off the ladder; Sol `medium` is not a rung. React and component legs go to headless Claude Opus `xhigh` (see claude-reviews.md), not to a native agent. A context fork is not a filesystem snapshot. Native agents share the directory and filesystem; explicitly tell a worker to set its command working directory to the assigned worktree. There is no native `isolation` argument in this schema. Verify the required worktree readiness first and include [check scope/log/handle rules](execution-checks.md) in the brief; native transport does not prevent a worker from discarding a yielded shell handle.
...
Use completion notifications as the primary signal. Wait for the longest interval active session wait/communication limits permit; a user update needs no status/log call. A timeout alone does not justify inspection. Inspect for a failure, a missed milestone/deadline, or evidence needed for a decision. If progress is normal, record the next expected milestone and return to waiting. Track expectations per task even when sibling messages arrive; do not repeatedly move a missed deadline without evidence.
