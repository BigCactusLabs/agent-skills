...
Implementation dispatch uses the full brief in SKILL.md: `gpt-5.6-sol` with `medium` for machine-checked legs, `gpt-6-astra` with `low` for review-checked legs. Terra is off the ladder; Sol `high` is not a rung. React and component legs go to headless Claude Opus `high` (see claude-reviews.md), not to a native agent. A context fork is not a filesystem snapshot. Native agents share the directory and filesystem; explicitly tell a worker to set its command working directory to the assigned worktree. There is no native `isolation` argument in this schema. Verify the required worktree readiness first and include [check scope/log/handle rules](execution-checks.md) in the brief; native transport does not prevent a worker from discarding a yielded shell handle.
