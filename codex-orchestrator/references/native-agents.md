...
The invoking Astra session remains the sole lead. An Astra `low` child is a bounded implementer with `fork_turns: "none"`, never another orchestrator; it cannot delegate or inherit the lead's full assignment.

## Dispatch and context

In the verified runtime, `spawn_agent` accepts `task_name`, `message`, `model`, `reasoning_effort`, and `fork_turns`. Names use lowercase letters, digits, and underscores. Build the spawn arguments from the current task record, with explicit model and effort; fail missing routing fields before calling the tool. Store the returned agent ID or canonical task path; it is the management handle. Record requested settings separately from runtime settings actually exposed by the harness, leaving the latter unknown if unavailable. Update the current manifest record after dispatch/result/ruling rather than only appending history.
...
Implementation dispatch uses the [worker brief](briefs-and-repairs.md#worker-brief): `gpt-5.6-sol` with `medium` for machine-checked legs, `gpt-6-astra` with `low` for review-checked legs. Terra is off the ladder; Sol `high` is not a rung. React and component legs go to headless Claude Opus `high` (see claude-reviews.md), not to a native agent. A context fork is not a filesystem snapshot. Native agents share the directory and filesystem; explicitly tell a worker to set its command working directory to the assigned worktree. There is no native `isolation` argument in this schema. Verify the required worktree readiness first and include [check scope/log/handle rules](execution-checks.md) in the brief; native transport does not prevent a worker from discarding a yielded shell handle.

Only dispatch a concrete independent leg while useful local work remains. Do not manufacture that local work or duplicate a worker's assignment to justify delegation. Once useful independent work is exhausted, wait. Launch bounded siblings up to capacity without waiting for each one's result. With four total runtime slots, Astra plus three workers fills the runtime; do not assume this cap on a different harness. An idle agent and a reusable handle do not by themselves prove that a new slot is available; follow the live capacity/status result.
...
`send_message` is not an idle-worker resume. Use `followup_task` after completion. Both continue the same handle and its recorded configuration; do not invent model/effort arguments on a follow-up or add a CLI process just to re-pin them. Neither tool changes a worker's model in this schema: a Luna-to-Sol or Sol-to-Astra escalation requires a new spawn, seeded with the result and failure evidence. Preserve the old worker's artifacts and account for live capacity before replacing it. Apply the [resume check](briefs-and-repairs.md#task-record-and-resume-check) before implementation steering or a new attempt; a fresh handle cannot reset repair counts or a user checkpoint.
...
Use completion notifications as the primary signal. Follow active session wait and communication limits; a user update does not require a status or diagnostic call. After two consecutive 60-second timeouts without new state, call `list_agents` once. If that establishes normal progress, record the next expected milestone and return to waiting; do not repeat the same inspection every two timeouts. Inspect again for a failure, a missed milestone/deadline, or evidence needed for a specific decision, even if sibling messages keep the mailbox active. Track expectations per task, not per mailbox; do not repeatedly move a missed deadline without evidence.

For an ended generation with a matching report/artifact, collect it; do not wait for another completion event. If a required report is missing, first inspect the saved current-generation artifacts and actual output. When writes/child commands have stopped and Astra can verify the current snapshot and acceptance checks, save a [lead recovery report](briefs-and-repairs.md#report-ownership-and-recovery) without waking the worker solely for a final message. Retain the ended/interrupted turn status; report collection and work acceptance are separate claims. If evidence is insufficient, allow one explicit follow-up requesting that generation's existing artifact/result without repeating completed work; label the recovery turn separately and state which generation it recovers. If that recovery fails, preserve the inconsistency, mark the report unavailable and acceptance failed/blocked, and stop the loop. If running, distinguish a known slow command from no observable progress using the task's expected duration and artifacts; silence alone is not a reason to kill it. At the task-specific deadline, inspect once and interrupt only if no useful progress or a safety/scope problem warrants it. Do not alternate identical sends, waits, and interrupts indefinitely.
...
The default independent reviewer remains restricted headless Claude. This section applies only when a native reviewer is otherwise authorized by the routing rules; it does not authorize a separate Astra judge. Use `fork_turns: "none"` for a fresh reviewer. Supply the user contract, base/diff, scope, and read-only constraints. Allow tests only where their generated outputs and resource use are permitted. Use the [review brief](briefs-and-repairs.md#verification-and-review-brief): require each finding to include a path, trigger, expected/observed results, impact, and evidence; label reasoning-only suspicions. Return the reviewed snapshot and closure status of prior finding IDs. Separate advisory and assigned integration work from acceptance findings. Where a remedy is clear, identify the smallest adequate closure and label alternatives as alternatives. Do not ask the reviewer to confirm another model's verdict.
