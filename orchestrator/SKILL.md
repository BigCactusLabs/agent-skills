...
| Claude reviewers | `pr-reviewer-high` default **and** judge on silent-failure domains; also plans, specs, and uncommitted changes the brief names | `-xhigh` only on a long-horizon near-miss; `pr-reviewer-max` **user-gated**: WebDev / visual-refinement review only |
