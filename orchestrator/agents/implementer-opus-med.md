...
description: General implementation worker at opus/medium, the first Claude implementation rung for all coding tasks, inside a pre-provisioned git worktree. Implements, verifies with the tests named in the brief, commits on its own branch. Never pushes, merges, or deploys.
model: opus
effort: medium
