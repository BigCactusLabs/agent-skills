...
description: Adversarial read-only code reviewer for a pull request diff. Opus at max effort. User-gated: dispatch only with explicit user permission, for WebDev / visual-refinement review, where max measurably beats high; elsewhere use pr-reviewer-high. Reads, greps, and may run tests to verify claims, but never edits files, never changes git state, and never comments on or merges the PR.
